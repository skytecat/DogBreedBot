...
- **Классификация** породы среди 120 классов датасета Stanford Dogs с использованием **transfer learning**: признаки извлекаются с помощью **ResNet18**, предобученной на **ImageNet**, а финальное решение принимает **Logistic Regression**, обученная на этих признаках (точность — 83.9% на валидации);
...
- **Прозрачная и пошаговая обратная связь**:
  ...
  Такой подход позволяет пользователю:
    - контролировать, какую именно собаку анализирует бот (особенно важно при нескольких животных в кадре)
    - оценивать корректность детекции (если рамка захватила не ту область, то можно прислать новое фото)
    - принимать решение о доверии к ответу на основе численной уверенности модели
...
Все этапы разработки модели - от анализа данных до подбора архитектуры были протестированы в Jupyter Notebook.

**Файл содержит:**
- Примеры изображений из датасета
- Тестирование различных моделей детекции (YOLOv8n vs YOLOv8s)
- Подбор гиперпараметров классификатора
- Визуализацию ошибок и уверенности модели

📁 Notebook: notebooks/dog_breed_bot.ipynb
...
[Исходное фото]
        ↓
[YOLOv8: детекция] → (если собака НЕ найдена → "Не вижу собаку")
        │
        | (собака найдена)
        ↓
[Обрезка по bbox]
...
[ResNet18 → признаки]
        ↓
[Logistic Regression → порода]
        ↓
[Ответ на русском + уверенность]
```
...
<img src="assets/analysis/detection/photo_0.jpg" alt="Демонстрация работы бота" width="400"/>

<img src="assets/analysis/detection/photo_1.jpg" alt="Демонстрация работы бота" width="400"/>

<img src="assets/analysis/detection/photo_2.jpg" alt="Демонстрация работы бота" width="400"/>
...
---
Ниже примеры случаев, когда модель **не смогла найти собаку на фото**

**Японский хин**

![Ошибка детекции](assets/analysis/detection/photo_3.jpg)

**Ксолоицкуинтли**

![Ошибка детекции](assets/analysis/detection/photo_4.jpg)

**Шотландская оленья борзая**

![Ошибка детекции](assets/analysis/detection/photo_5.jpg)


### Оценка качества классификации

Модель оценивалась на валидационной выборке (10% от Stanford Dogs, 120 пород).  
`Точность классификации: 83.9%`  

#### Распределение уверенности

На графике ниже распределение уверенности модели для **правильных** и **ошибочных** предсказаний:

![Уверенность модели](assets/analysis/classification/confidence_histogram.png)

> - Модель уверенно правильно классифицирует большинство объектов
> - Ошибки происходят в основном там, где модель не уверена
> - Почти нет **"уверенных ошибок"**. Это снижает риск ложных срабатываний в продакшене

---

#### Примеры правильных предсказаний

Ниже реальные примеры, на которых модель точно определила породу собаки. Изображения демонстрируют разнообразие условий: от студийных фото до снимков в естественной среде.

Модель уверенно распознаёт как очень распространённые породы (например, Сибирский хаски), так и редкие или визуально сложные (Африканская охотничья собака, Брабанкон гриффон).

Уверенность модели колеблется **от 52% до 99%**, что говорит о её способности работать даже с неидеальными условиями и при этом сохраняя высокую точность.

Эти примеры подтверждают, что система надёжна и готова к использованию в реальных сценариях

![Правильные предсказания](assets/analysis/classification/correct_predictions.jpg)
...
- **Accuracy классификатора** (доля верных предсказаний среди найденных собак) = **84%**

```
[100 фото с собаками]
   ├── 89% → передано на классификацию
   │      ├── 84% → ✅ правильный ответ (75%)
   │      └── 16% → ❌ ошибка породы (14%)
   └── 11% → 🚫 пропущено (11%)
```
...
$$
0.89 \times 0.84 = \mathbf{74.8\%}
$$

> Это означает: из **100** фото с настоящими собаками бот выдаст **правильный ответ в 75 случаях**
